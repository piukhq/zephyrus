...
class TestJwtAuth(TestCase):
    ...
    client_obj = {"client_id": "testid", "secret": "testsecret", "organisation": "The Organisation"}
    ...
    def test_auth_endpoint_success(self, mock_client_info, mock_gen_jwt):
        ...
        mock_client_info.return_value.get_client.return_value = {"client_id": "testid", "secret": "testsecret"}
    ...
    def test_auth_endpoint_fails_invalid_secret(self, mock_client_info, mock_gen_jwt):
        mock_client_info.return_value.data = [{"client_id": "testid", "secret": "bad_secret"}]
        mock_gen_jwt.return_value = "really_bad_jwt"
        resp = self.simulate_post(self.amex_auth_end_point, json=self.payload, headers={})
        self.assertEqual(resp.status_code, 401)
    ...
    def test_generate_jwt(self):
        ...
        claims = jose.jwt.decode(token, key=settings.SIGNATURE_SECRET, audience="https://api.bink.com", issuer="bink")
