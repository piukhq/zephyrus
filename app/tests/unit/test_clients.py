...
class TestClientAppInfo(TestCase):
    ...
    @mock.patch('requests.get', autospec=True)
    @mock.patch.object(ClientInfo, 'is_stale')
    def test_update_client_raises_custom_exception_on_connection_error(self, mock_is_stale, mock_request,
                                                                       mock_redis_set):
        mock_is_stale.return_value = True
    ...
    @mock.patch('requests.get', autospec=True)
    @mock.patch.object(ClientInfo, 'is_stale')
    def test_update_client_raises_exception_on_unexpected_response(self, mock_is_stale, mock_request,
                                                                   mock_redis_set):
        mock_is_stale.return_value = True
