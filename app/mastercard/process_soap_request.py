...
def get_xml_element(element_tree, element_tag):
    for element in element_tree.tree.iter():
...
def remove(element_tree, element_tag):
        element = get_xml_element(element_tree, element_tag)
        parent = element.getparent()
        parent.remove(element)


def get_valid_signed_data(binary_xml, root_cert):
    assertion_data = XMLVerifier().verify(binary_xml, x509_cert=root_cert).signed_xml
...
def mastercard_request(xml_data):
    try:
        ...
        new_tree = remove(element_tree, "Signature"),mc_data, False, None

        return etree.tostring(new_tree).decode('utf8')
